...
async function infosauth(interaction, client) {
    ...
    const embed = new EmbedBuilder().setTitle(`ðŸ’¨ ”” Painel de info`)
        .setColor("Blue")
        .setDescription(`Olá, senhor(a) ${interaction.user}, esta são as suas informações!`)
        .addFields(
            {
                name: "ðŸ”— URL do Backup:",
    ...
    await interaction.update({ content: ``, embeds: [embed], flags: [Discord.MessageFlags.Ephemeral], components: [row2] })
}
