...
function MessageStock(interaction, stat, prod, camp, update, reply) {
    ...
    if (stat == 1) {
        if (update !== true) {
            interaction.reply({ embeds: [], content: `Selecione o método`, components: [row2, row3], flags: [Discord.MessageFlags.Ephemeral], fetchReply: true }).then(async msg => {
            ...
            if (reply !== true) {
                interaction.update({ embeds: [], content: `Selecione o método`, components: [row2, row3], flags: [Discord.MessageFlags.Ephemeral], fetchReply: true }).then(async msg => {
