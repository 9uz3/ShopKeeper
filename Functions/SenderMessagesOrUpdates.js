...
function getEmojiMsg() {
    ...
    const sorted = [...Entrega2].sort((a, b) => {
        const numA = parseInt(a.name.replace('ea', ''), 10);
        const numB = parseInt(b.name.replace('ea', ''), 10);
        return numA - numB;
    });
    return sorted.map(element => `<:${element.name}:${element.id}>`).join("")
}
...
async function MessageCreate(interaction, client) {
    ...
    if (fdfd.textobutton == undefined) {
        ...
        if (yyy.Config.entrega == 'Sim') {
            const emojiMsg1 = getEmojiMsg()
            if (emojiMsg1 !== '') {
                embed.setTitle(emojiMsg1)
            }
        }
        ...
        if (yyy.Campos.length <= 1) return interaction.update({ content: `Faça o processo de envio novamente!`, flags: [Discord.MessageFlags.Ephemeral], components: [] })
        ...
        try {
            await interaction.update({ embeds: [], components: [], content: '🔄 Aguarde...' }).then(async msgg => {
                ...
                await channel.send({ embeds: [embed], components: [style2row] }).then(async msggg => {
                    ...
                    await msgg.edit({ content: `✅ Mensagem postada!`, flags: [Discord.MessageFlags.Ephemeral], components: [row4] })
            ...
            await interaction.followUp({ content: `âŒ | Ocorreu um erro ao enviar a mensagem.\n\`${mensagem}\``, flags: [Discord.MessageFlags.Ephemeral] })
...
if (yyy.Config.entrega == `Sim`) {
            const emojiMsg2 = getEmojiMsg()
            if (emojiMsg2 !== '') {
                embed.setTitle(emojiMsg2)
            }
        }
        ...
        try {
            await interaction.update({ embeds: [], components: [], content: '🔄 Aguarde...' }).then(async msgg => {
                ...
                await channel.send({ embeds: [embed], components: [row2] }).then(msggg => {
                    ...
                    msgg.edit({ content: `✅ Mensagem postada!`, flags: [Discord.MessageFlags.Ephemeral], components: [row4] })
                })
            ...
            interaction.followUp({ content: `âŒ | Ocorreu um erro ao enviar a mensagem.\n\`${mensagemTraduzida}\``, flags: [Discord.MessageFlags.Ephemeral] })
        }
...
async function UpdateMessageProduto(client, produto) {
    ...
    if (ghgh.Config.entrega == 'Sim') {
            const emojiMsg = getEmojiMsg()
            if (emojiMsg !== '') {
                embed.setTitle(emojiMsg)
        }
    }
    ...
    if (ghgh.Campos.length > 1) {
        ...
        if (ghgh.Config.entrega == 'Sim') {
            const emojiMsg3 = getEmojiMsg()
            if (emojiMsg3 !== '') {
                embed22.setTitle(emojiMsg3)
            }
        }
