...
async function GerenciarCampos2(interaction, campo, produtoname, update, reply) {
    ...
    if (produtoname == undefined) {
        ...
        if (update !== true) {
            await interaction.reply({ embeds: [embed], components: [row2, row3, row5], content: ``, fetchReply: true, flags: [Discord.MessageFlags.Ephemeral] }).then(async msg => {
            ...
            if (reply !== true) {
                ...
                await interaction.reply({ embeds: [embed], components: [row2, row3, row5], content: ``, flags: [Discord.MessageFlags.Ephemeral] }).then(async msg => {
...
async function GerenciarCampos(interaction, produtoname) {
    ...
    const hasCampos = aaaaaa.length > 0;
    const components = hasCampos ? [row2, row3, row4] : [row3, row4];
    const payload = { components, embeds: [embed], content: `` };
    ...
    if (interaction.isModalSubmit()) {
        if (interaction.deferred || interaction.replied) {
            ...
            const msg = await interaction.reply({ ...payload, flags: [Discord.MessageFlags.Ephemeral] });
            await db.set(msg.id, { name: produtoname });
        }
        return;
    }
