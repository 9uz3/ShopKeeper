...
const { getMpApi } = require("../config");
const db = new QuickDB();


async function DentroCarrinhoPix(interaction, client) {
    await interaction.deferUpdate()
    await interaction.message.edit({ content: `🔄 Aguarde...`, flags: [Discord.MessageFlags.Ephemeral], components: [] }).then(async tt => {
        ...
        var novaDataFormatada = agora.toISOString().replace('Z', '-03:00');
        ...
        var payment_data = {
            ...
            payer: {
                email: `${interaction.user.id}@store.user`,
        ...
        const clientMP = new MercadoPagoConfig({ accessToken: getMpApi() });
        ...
        await payment.create({ body: payment_data })
           ...
           //     interaction.channel.setName(`💰・${yy.user.username}・${yy.user.id}`)
            ...
            .catch(function (error) {
                ...
                tt.edit({ content: `Selecione uma forma de pagamento.`, flags: [Discord.MessageFlags.Ephemeral], components: [row3] })
                interaction.followUp({ content: `âŒ | Ocorreu um erro ao criar o pagamento, tente novamente.\nError: ${error}`, flags: [Discord.MessageFlags.Ephemeral] })
            })
...
function DentroCarrinho2(interaction) {
    ...
    if (yd.quantidadeselecionada > gggaaa.condicao?.valormaximo) return interaction.reply({ content: `âŒ | Você não pode comprar mais de \`${gggaaa.condicao.valormaximo}x ${yd.infos.produto} - ${yd.infos.campo}\``, flags: [Discord.MessageFlags.Ephemeral] })
    if (yd.quantidadeselecionada < gggaaa.condicao?.valorminimo) return interaction.reply({ content: `âŒ | Você não pode comprar mais de \`${gggaaa.condicao.valorminimo}x ${yd.infos.produto} - ${yd.infos.campo}\``, flags: [Discord.MessageFlags.Ephemeral] })
