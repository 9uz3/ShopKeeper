        ...
        try {
            if (interaction.isButton()) {
                if (interaction.customId === 'configlock') {
                    ...
                    await interaction.update({ content: '', embeds: [embed], components: [actionRow, actionRow2], flags: [Discord.MessageFlags.Ephemeral] });
                }
            }
            ...
            if (interaction.isButton()) {
                ...
                if (interaction.customId === 'disableConfig') {
                    ...
                    if (automaticos[guildId]) {
                        ...
                        await interaction.reply({ content: 'Configuração de bloqueio automático desativada.', flags: [Discord.MessageFlags.Ephemeral] });
                    } else {
                        await interaction.reply({ content: 'Nenhuma configuração de bloqueio automático encontrada para desativar.', flags: [Discord.MessageFlags.Ephemeral] });
                    }
                }
            }
            ...
            if (interaction.isModalSubmit()) {
                if (interaction.customId === 'configurarBloqueio') {
                    ...
                    if (!isValidTime(lockTime) || !isValidTime(unlockTime)) {
                        await interaction.reply({ content: 'Horário inválido. Use o formato HH:mm.', flags: [Discord.MessageFlags.Ephemeral] });
                        return;
                    }
                    ...
                    if (invalidIds.length > 0) {
                        await interaction.reply({ content: `ID(s) de canal inválido(s): ${invalidIds.join(', ')}`, flags: [Discord.MessageFlags.Ephemeral] });
                        return;
                    }
            ...
            await interaction.reply({ content: 'Ocorreu um erro ao processar sua solicitação. Tente novamente mais tarde.', flags: [Discord.MessageFlags.Ephemeral] });
        }
    }
};
