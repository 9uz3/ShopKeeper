    ...
    run: async (interaction, client) => {
            ...
            if (interaction.customId === '2313141') {
                ...
                if (isNaN(qtd) || qtd <= 0 || qtd % 1 !== 0) {
                    return interaction.reply({
                        content: `â“ A quantidade \`${qtd}\` não é um número inteiro válido ou é menor ou igual a zero, tente novamente.`,
                ...
                if (qtd > gggaaa.estoque.length) {
                    return interaction.reply({
                        content: `âŒ A quantidade solicitada de \`${qtd}\` excede o estoque disponível.`,
                        flags: [Discord.MessageFlags.Ephemeral]
                    });
                }
                ...
                if (ggg.cupomadicionado !== undefined) {
                    ...
                    if (gggaaa.condicoes?.precominimo !== undefined) {
                        ...
                        if (qtd < gggaaa.condicoes?.precominimo) {
                            return interaction.reply({ content: `âŒ A quantidade solicitada de \`${qtd}\` não está no valor minimo para utilizar o cupom de \`${gggaaa.condicoes?.precominimo}\`.`, flags: [Discord.MessageFlags.Ephemeral] })
                        }

                        if (qtd > gggaaa.condicoes?.qtdmaxima) {
                            return interaction.reply({ content: `âŒ A quantidade solicitada de \`${qtd}\` excede o limite para o uso do cupom de \`${gggaaa.condicoes?.precominimo}\`.`, flags: [Discord.MessageFlags.Ephemeral] })
                        }
        ...
        if (interaction.isButton()) {
            ...
            if (interaction.customId == 'codigocopiaecola') {
                ...
                interaction.reply({ content: `${yy.pagamentos.cp}`, flags: [Discord.MessageFlags.Ephemeral] })
            ...
            if (interaction.customId === 'xxxxxxxxxxxxxxxxxxxxxxxxxxxxx') {
                try {
                  ...
                  await interaction.reply({ content: `${tttttt.pixcopiaecola}`, flags: [Discord.MessageFlags.Ephemeral] });
                } catch (error) {
                }
              }
            ...
            if (interaction.customId == 'confirmarpagamentomanual') {
                ...
                if (carrinhos.has(interaction.channel.id) == false) return interaction.reply({ content: `âŒ Não há um carrinho aberto neste canal.`, flags: [Discord.MessageFlags.Ephemeral] })
                ...
                interaction.reply({ content: `✅ Pagamento aprovado manualmente. Aguarde..`, flags: [Discord.MessageFlags.Ephemeral] })

            }

            if (interaction.customId == 'codigocopiaecolaadwdawd') {
                const pagamento = configuracao.get(`pagamentos.SemiAutomatico`)
                interaction.reply({ content: `Chave pix: ${pagamento.pix}`, flags: [Discord.MessageFlags.Ephemeral] })
            }
            ...
            if (interaction.customId.startsWith('comprarid_')) {
                ...
                if (hhhh == null) return interaction.reply({ content: `âŒ Este produto não existe.`, flags: [Discord.MessageFlags.Ephemeral] }).then(msg => {
                    interaction.message.delete()
                })
            ...
            if (interaction.customId.startsWith('foraestoquealarme_')) {
                ...
                if (gggaaa.avisar !== undefined) {
                    if (!gggaaa.avisar.includes(interaction.user.id)) {
                        ...
                        if (status == 1) {
                            interaction.reply({ content: `✅ Você será notificado assim que for reabastecido o stock (Para desabilitar clique abaixo).`, flags: [Discord.MessageFlags.Ephemeral] })
                        } else {
                        ...
                        if (status == 1) {
                            interaction.reply({ content: `â— Você já estava salvo para ser notificado, portanto foi retirado (Caso queira basta clicar abaixo).`, flags: [Discord.MessageFlags.Ephemeral] })
                        } else {
                            interaction.update({ content: `â— Você já estava salvo para ser notificado, portanto foi retirado (Caso queira basta clicar abaixo).` })
                        }
        ...
        if (Object.keys(infos).length !== 0) {
            ...
            if (verify.error == 400) {
                ...
                interaction.reply({ content: `Este item está fora de estoque.`, flags: [Discord.MessageFlags.Ephemeral], components: [row3] })
            ...
            if (gggaaa.condicao?.idcargo !== undefined) {
                ...
                if (temCargo == false) return interaction.reply({ content: `âŒ Você não possui permissão para comprar esse produto!`, flags: [Discord.MessageFlags.Ephemeral] })
            }
