    ...
    run: async (interaction, client) => {
        ...
        if (interaction.type == Discord.InteractionType.ModalSubmit) {
            if (interaction.customId === 'ConfigurarPagamentoManual2') {
                ...
                if (a1 !== 'não' && a1 !== 'sim') {
                    return interaction.reply({ flags: [Discord.MessageFlags.Ephemeral], content: `âŒ Confirmação não validada.` });
                }
                ...
                interaction.followUp({ content: `✅ Configuração salva.`, flags: [Discord.MessageFlags.Ephemeral] })
        ...
        if (interaction.isButton()) {
            ...
            if (interaction.customId == 'ConfigurarPagamentoManual') {
                ...
                const newnameboteD = new TextInputBuilder()
                    ...
                    .setLabel(`MENSAGEM APÓS REQUISIÇíO DO PEDIDO`)
            ...
            if (interaction.customId == 'remcampo') {
                ...
                if (!ggg?.length) {
                    return interaction.reply({ flags: [Discord.MessageFlags.Ephemeral], content: `❌ Esse produto ainda não possui nenhum campo.` });
                }
                ...
                try {
                    ...
                    await interaction.update({ components: [style2row], content: `â“ Quais campos de \`${ggg22.Config.name}\` deseja remover?`, embeds: [] })
                } catch (error) {
                }
            }
            ...
            if (interaction.customId == 'addcampoo') {
                const ggg2 = await db.get(interaction.message.id)
                if (!ggg2?.name) {
                    return interaction.reply({ flags: [Discord.MessageFlags.Ephemeral], content: `❌ Sessão expirada. Abra o gerenciamento do produto novamente.` })
                }

                const ggg = produtos.get(`${ggg2.name}.Campos`) || []
                if (ggg.length >= 24) return interaction.reply({ flags: [Discord.MessageFlags.Ephemeral], content: `❌ Você já adicionou o máximo de campos possível nesse produto.` })
        ...
        if (interaction.type == Discord.InteractionType.ModalSubmit) {
            if (interaction.customId === 'duigawd8wa8dtgvaw7') {
                ...
                if (confirm !== 'sim') return interaction.reply({ content: `âŒ Confirmação não validada.`, flags: [Discord.MessageFlags.Ephemeral] })
            ...
            if (interaction.customId === 'sdaju112341111idsjjsdua') {
                let nomecampo = interaction.fields.getTextInputValue('tokenMP').trim();
                let desccampo = interaction.fields.getTextInputValue('tokenMP2');
                let precocampo = interaction.fields.getTextInputValue('tokenMP3').trim().replace(',', '.');
                ...
                if (!nomecampo) {
                    return interaction.reply({ flags: [Discord.MessageFlags.Ephemeral], content: `❌ Informe um nome para o campo.` });
                }

                if (isNaN(precocampo)) {
                    return interaction.reply({ flags: [Discord.MessageFlags.Ephemeral], content: `❌ Preço inserido \`${precocampo}\` inválido.` });
                }

                const campos = produtos.get(`${ggg.name}.Campos`) || [];
                if (campos.some((campo) => campo.Nome === nomecampo)) {
                    return interaction.reply({ flags: [Discord.MessageFlags.Ephemeral], content: `❌ Já existe um campo com esse nome neste produto.` });
                }
                ...
                await interaction.deferReply({ flags: [Discord.MessageFlags.Ephemeral] });
                await GerenciarCampos(interaction, ggg.name);
                await interaction.followUp({ content: `✅ Campo adicionado.`, flags: [Discord.MessageFlags.Ephemeral] });
                await UpdateMessageProduto(client, ggg.name);
            ...
            if (interaction.customId === 'sdaju11111idsjjsdua') {
                ...
                if (produtos.get(`${nome}`) !== null) return interaction.editReply({ content: `âŒ | Opss, ja existe um produto com esse nome.` })//
