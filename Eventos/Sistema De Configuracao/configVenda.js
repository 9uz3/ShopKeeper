    ...
    run: async (interaction, client) => {
        ...
        if (interaction.type == Discord.InteractionType.ModalSubmit) {
            if (interaction.customId === 'awdawdawdawdadawdawfewfryty565') {
                ...
                if (a4 !== '') {
                    ...
                    if (!hexColorRegex.test(a4)) return interaction.reply({ content: `âŒ | Código Hex Color inválido.`, flags: [Discord.MessageFlags.Ephemeral] })
                }

                const ggg = await db.get(interaction.message.id)

                if (a3 !== '') {
                    if (a3 !== 'verde' && a3 !== 'azul' && a3 !== 'cinza' && a3 !== 'vermelho') return interaction.reply({ content: `Você interagiu incorretamente no estilo do Button`, flags: [Discord.MessageFlags.Ephemeral] })
                } else {
                    a3 = 'cinza'
                }
            ...
            if (interaction.customId.startsWith('wdawdawdawdwadadsadawdwadwdw')) {
                ...
                let a1 = interaction.fields.getTextInputValue('tokenMP');
                if (a1 !== 'sim') return interaction.reply({ content: `âŒ | Ação não validada para realizar reembolso.`, flags: [Discord.MessageFlags.Ephemeral] })
                const id = interaction.customId.split('_')[1]

                await interaction.reply({ content: `✅ | Estornando pagamento...`, flags: [Discord.MessageFlags.Ephemeral] })

                const axios = require('axios');
                const refundResponse = await axios.post(`https://api.mercadopago.com/v1/payments/${id}/refunds`, {}, {
                    headers: {
                        'Authorization': `Bearer ${getMpApi()}`
                    }
                });
        ...
        if (interaction.isButton()) {
            ...
            if (interaction.customId == 'syncproduto') {
                ...
                await interaction.reply({ content: `🔄 Sincronizando mensagens...`, flags: [Discord.MessageFlags.Ephemeral] }).then(async msg => {
            ...
            if (interaction.customId == 'colocarvenda') {
                ...
                if (gg2.length == 0) return interaction.reply({ content: `âŒ | Nenhum campo foi configurado.`, flags: [Discord.MessageFlags.Ephemeral] })
