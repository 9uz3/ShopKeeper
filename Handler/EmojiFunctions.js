...
function emojiOrFallback(guild, emojiId, fallback) {
    if (emojiId && guild?.emojis?.cache?.has(String(emojiId))) {
        return String(emojiId);
    }
    return fallback;
}

function setButtonEmoji(button, guild, emojiId, fallback) {
    const emoji = emojiOrFallback(guild, emojiId, fallback);
