...
module.exports = {
  ...
  run: async (client, interaction, message) => {
    ...
    if (rank.length === 0) {
      return interaction.reply({ content: `âŒ Não há dados suficientes para gerar um ranking.`, flags: [Discord.MessageFlags.Ephemeral] });
    }
    ...
    const updateMessage = async () => {
      ...
      try {
        await interaction.reply({ embeds: [embed], components: [row], flags: [Discord.MessageFlags.Ephemeral] });
      } catch (error) {
        await interaction.editReply({ embeds: [embed], components: [row], flags: [Discord.MessageFlags.Ephemeral] });
      }
