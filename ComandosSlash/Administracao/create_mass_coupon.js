...
module.exports = {
  ...
  run: async (client, interaction, message) => {
    ...
    if (discount > 100) return interaction.reply({ content: 'O desconto não pode ser maior que 100%', flags: [Discord.MessageFlags.Ephemeral] })
    if (discount < 0) return interaction.reply({ content: 'O desconto não pode ser menor que 0%', flags: [Discord.MessageFlags.Ephemeral] })
    if (days < 0) return interaction.reply({ content: 'Os dias não podem ser menor que 0', flags: [Discord.MessageFlags.Ephemeral] })
    if (max_uses < 0) return interaction.reply({ content: 'O máximo de usos não pode ser menor que 0', flags: [Discord.MessageFlags.Ephemeral] })
    if (amount < 0) return interaction.reply({ content: 'A quantidade não pode ser menor que 0', flags: [Discord.MessageFlags.Ephemeral] })
    if (buy_amount_max < 0) return interaction.reply({ content: 'A quantidade de compra máxima não pode ser menor que 0', flags: [Discord.MessageFlags.Ephemeral] })
    if (buy_amount < 0) return interaction.reply({ content: 'A quantidade de compra não pode ser menor que 0', flags: [Discord.MessageFlags.Ephemeral] })
